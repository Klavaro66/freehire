...
type Settings struct {
	...
	// RedisURL backs the shared rate limiter (internal/api/ratelimit). Unlike Meili,
	...
	// mask PII out of CV text before it reaches the LLM (internal/candidate/pii). Optional here, but
	...
	// S3 backs résumé storage (internal/platform/blobstore). Optional and provider-agnostic:
	...
	// (internal/candidate/cv). Optional: an empty value disables PDF rendering (the /me/cvs/:id/pdf
	...
	// (internal/candidate/matchanalysis). Override any field via MATCH_ANALYSIS_*; values
	...
	// (internal/platform/observability). Optional: an empty SentryDSN disables the integration
