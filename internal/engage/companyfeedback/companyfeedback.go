...
// pseudonymous persona (internal/engage/community's handle, not a real name), one row
// per (user, company, category), editable by resubmitting within that category
// (upsert-in-place, the same shape internal/engage/vote's company thumbs use). A user
...
// see internal/engage/report for the fuller resolve/dismiss/notify version of that
...
// internal/engage/report's maxDetailsLen, and for the same reason: long enough for a
...
// without a second round trip — the same shape as internal/engage/vote.Result.
type Summary struct {
	Count     int32
	RatingAvg *float32
}
...
// first use — the same site-wide persona internal/engage/community's discussion
...
// resolve/dismiss state (see internal/engage/report for that fuller shape, sized for
