...
//
// Originally internal/handler's userLLM, unexported and callable only from Fiber handlers.
// Extracted here once cmd/auto-apply became a second real caller with the same ownership
// shape (a specific candidate's own work, not owner-less background work) but no reason to
// import internal/handler — see openspec/changes/auto-apply-llm-drafting/design.md.
func Bind(ctx context.Context, keys *Resolver, client *llm.Client, userID int64, tags ...string) *llm.Client {
	if client == nil {
		return nil
	}
