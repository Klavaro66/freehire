...
`Bind`, in this package (`bind.go`) — moved here from `internal/handler`'s `userLLM` once
`cmd/auto-apply` became a second caller with no reason to import `internal/handler`.
...
1. Resolve through `Bind` (this package, `bind.go`) — the single identifier to grep for.
   `internal/handler`'s `llmBinding.bind` is a thin per-request wrapper over it, not a
   second implementation.
2. Give it a `feature:` tag — the constants live beside each caller (`internal/handler/
   user_llm.go` for request-driven surfaces, `tagAutoApplyDrafting` in
   `internal/atsapply/client.go` for `cmd/auto-apply`). One tag per thing a person (or a
   queue-driven attempt on their behalf) can ask for; do not tag two surfaces the same or
   the report stops answering the question it exists for.
