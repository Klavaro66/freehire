...
// (see internal/ingest/linkimport) is ingested as a normal public job, identical to any other
// crawled posting. A URL only the generic JSON-LD fallback can read, or plain text, becomes
// a new private job (see internal/job/privatejob) — visible only to its creator, excluded from
...
// resolveURL branches on which adapter matched — see internal/ingest/linkimport.Resolve — not on
