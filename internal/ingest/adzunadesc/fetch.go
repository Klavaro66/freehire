...
// Declared here rather than imported to mirror internal/ingest/applyform's own asGone, which notes
// why: the dependency runs the other way in that package. Here it is only mirrored for
// consistency, since internal/ingest/sources does not import this package.
...
// applyform's Lever fetcher and internal/ingest/linksource's generic resolver already use.
...
func FetchDescription(ctx context.Context, t Transport, url string) (string, error) {
	...
	// Unescape BEFORE sanitizing, matching internal/ingest/linksource's generic resolver: some
