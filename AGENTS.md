...
- `auto-apply` — drains `auto_apply_queue`: resolves a candidate's known profile answers against a job's live application form via a headless Chrome (`internal/atsapply`, `chromedp` — no second language or process) and submits only when every required question is answered; otherwise it touches nothing on the page. Needs `DATABASE_URL`, `S3_*` (résumé storage, wired but not yet used for uploads — see the package's own AGENTS.md) and a Chrome/Chromium binary on the host. What populates the queue is a separate, not-yet-built concern — this worker only ever drains what is already there. See [internal/autoapply/AGENTS.md](internal/autoapply/AGENTS.md) and [internal/atsapply/AGENTS.md](internal/atsapply/AGENTS.md).
