...
- `auto-apply` — drains `auto_apply_queue`: resolves a candidate's known profile answers against a job's live application form via a headless Chrome (`internal/atsapply`, `chromedp` — no second language or process) and submits only when every required question is answered; otherwise it touches nothing on the page. Needs `DATABASE_URL` and a Chrome/Chromium binary on the host; `S3_*` is read but not required — résumé uploads aren't wired up yet (see `internal/atsapply/AGENTS.md`'s known gaps), so an unconfigured object store degrades the same way it does elsewhere in this fleet rather than blocking startup. What populates the queue is a separate, not-yet-built concern — this worker only ever drains what is already there. See [internal/autoapply/AGENTS.md](internal/autoapply/AGENTS.md) and [internal/atsapply/AGENTS.md](internal/atsapply/AGENTS.md).
...
| **Auto-apply** (unattended submission: queue-drain domain logic, and the headless browser driver) | [internal/autoapply/AGENTS.md](internal/autoapply/AGENTS.md), [internal/atsapply/AGENTS.md](internal/atsapply/AGENTS.md) |
