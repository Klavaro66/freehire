...
- `backfill-link-contributions` — one-off: carries the crowdsourced contributions #2357 left behind. That change moved the lifecycle out of `link_contributions` into `boards` + `board_submissions` — the read and write paths, but not the DATA, so on prod 404 rows from 11 users stayed put while `boards` carried 3 of them. 401 stopped showing on their submitters' contributions page and 28 stopped being actionable. **It is the precondition for dropping `link_contributions`**, which the board-catalog design asks for and which would otherwise destroy those 28. `review` rows become `board_submissions`, `pending` rows become pending catalog boards, `onboarded` rows ATTRIBUTE the existing catalog row instead of adding a second, and `rejected` rows are deliberately NOT carried — migration 0049 already freed their identity, so the board can be re-contributed and judged again, and `boards`' own `rejected` means something else (failed insert-time validation). Original `created_at` is preserved, or every user's list would reorder to today. Reports by default, `--apply` writes, idempotent. Needs only `DATABASE_URL`.
...
- `auto-apply` — drains `auto_apply_queue`: resolves a candidate's known profile answers against a job's live application form via a headless Chrome (`internal/api/atsapply`, `chromedp` — no second language or process) and submits only when every required question is answered; otherwise it touches nothing on the page. Needs `DATABASE_URL` and a Chrome/Chromium binary on the host; `S3_*` is read but not required — résumé uploads aren't wired up yet (see `internal/api/atsapply/AGENTS.md`'s known gaps), so an unconfigured object store degrades the same way it does elsewhere in this fleet rather than blocking startup. What populates the queue is a separate, not-yet-built concern — this worker only ever drains what is already there. See [internal/application/autoapply/AGENTS.md](internal/application/autoapply/AGENTS.md) and [internal/api/atsapply/AGENTS.md](internal/api/atsapply/AGENTS.md).
