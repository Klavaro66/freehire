# ai-role-archetype Specification

## Purpose
TBD - created by archiving change refine-ai-role-classification. Update Purpose after archive.
## Requirements
### Requirement: A deterministic rule set derives a job's AI skill-signature archetype

The system SHALL provide `internal/ai/aiarchetype`, a curated deterministic rule
set (mirroring the dict-only doctrine of `internal/dict/classify`,
`internal/dict/roletag`, and `internal/dict/skilltag`) that derives a job's
...
generated from the same source of truth as `internal/ai/aiarchetype`'s rule
table rather than hand-duplicated. `vocab.AIArchetypeValues` SHALL be
cross-checked by a test against the archetype slugs `internal/ai/aiarchetype`'s
...
- **THEN** it contains exactly the six archetype slugs `internal/ai/aiarchetype`
...
- **WHEN** `vocab.AIArchetypeValues` and `internal/ai/aiarchetype`'s rule-table
