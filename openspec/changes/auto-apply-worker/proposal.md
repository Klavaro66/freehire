...
- On a successful submission, the worker records it through `db.Queries.MarkJobApplied` (the same statement `jobtracking`'s own manual-apply path runs) inside one transaction with retiring the queue entry, so `user_jobs`/`application_events` never diverge between an automated and a manual application, and a reprocessed attempt cannot double-submit.
