...
- [ ] 1.1 Add `Requirement{Text, Priority string}` and
...
- [ ] 1.2 Add `maxRequirements`/`maxRequirementTextRunes` consts.
- [ ] 1.3 Implement `boundRequirements` (mirrors `boundCities`: cap list
...
- [ ] 1.4 Extend `TestRoundTripFidelity` in `enrichment_test.go` with a
      `Requirements` value in the `original` fixture.
- [ ] 1.5 Add `TestSanitizeBoundsRequirements` in `enrichment_test.go`:
...
- [ ] 3.2 Add `"requirements"` to the field list in
      `TestRequestSchema_CarriesTheFieldsThePromptAsksFor` (`schema_test.go`).
- [ ] 3.3 Add `TestRequestSchema_RequirementsHasNoEnum` asserting
      `items.properties.priority` on the `requirements` schema node carries
      no `"enum"` key.

## 4. Prompt

- [ ] 4.1 Add an unconditional instruction block to `buildSystemPrompt` in
...
- [ ] 4.2 Add `"requirements"` to the field list in
      `TestSystemPromptKeepsServedAndHybridFields` (`langchain_test.go`).
- [ ] 4.3 Add a test asserting the prompt contains the "no CV to compare
      against" framing.

## 5. Public API pass-through

- [ ] 5.1 Add a test in `internal/jobview` asserting a job's `Requirements`
...
- [ ] 6.1 Run `make gen-contracts` and commit the regenerated
      `web/src/lib/generated/contracts.ts` (new `Requirement` interface,
      `requirements` member on `Enrichment`).
- [ ] 6.2 Run `gofmt -l .` (must be clean), `go vet ./...`, `go test ./...`.
- [ ] 6.3 Run `go vet -tags=integration ./...`.
