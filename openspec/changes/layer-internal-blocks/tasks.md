...
- [ ] 2.1 Move the `llm.Settings` conversion out of `internal/config`. `internal/config/llm.go:75` defines `func (l LLM) Settings(model string) llm.Settings`; it is the only reason `config` imports `llm`. Move the conversion into `internal/llm` (or the `cmd/*` wiring); `internal/config` keeps the env struct. Verify `internal/config` no longer imports `internal/llm`.
