...
fetching per group or per subscription. Add `ExcludedSkillsByUserIDs(ctx, userIDs []int64)
(map[int64][]string, error)` to `internal/userprofile` (backed by one
`SELECT user_id, excluded_skills FROM user_profiles WHERE user_id = ANY($1)`), and have `Runner.match`
call it once after `ListActiveSubscriptions`, then pass the resulting map down into `matchQuery`. A
user id absent from the map (no profile row yet) is treated as an empty exclude set — the same default
as everywhere else `excluded_skills` is read.
