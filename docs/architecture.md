...
**The API.** One Fiber server, one public wire shape. `internal/job/jobview` owns the single JSON representation of a job used by the list endpoint, the detail endpoint, and the search index alike, so those surfaces cannot drift apart. Responses use `{"data": ...}` for single items, `{"data": ..., "meta": {...}}` for lists, and `{"error": msg}` for failures. The catalogue is public and keyless — `GET https://freehire.me/api/v1/jobs` needs no credential.

Deeper: [`internal/ingest/pipeline`](../internal/ingest/pipeline/AGENTS.md) · [`internal/ingest/sources`](../internal/ingest/sources/AGENTS.md) · [`internal/search`](../internal/search/AGENTS.md) · [`internal/search/searchdrain`](../internal/search/searchdrain/AGENTS.md) · [`internal/platform/db`](../internal/platform/db/AGENTS.md) · [`internal/api/handler`](../internal/api/handler/AGENTS.md) · [`internal/job/jobview`](../internal/job/jobview/AGENTS.md)
...
**`internal/`** is where the domain lives, one package per concern — and each package sits in exactly one of eleven **blocks**, which form eight layers. Reading bottom-up: `platform` (db, worker plumbing, transport — nothing here knows what a vacancy is), `dict` (the facet dictionaries), `ai` and `identity`, `candidate`, `job`, `application` and `search`, `engage` and `ingest`, and `api` at the top. A block may import only blocks strictly below it, and two blocks sharing a layer may not see each other at all. `depguard` fails on the offending import line; [`internal/platform/arch/layering`](../internal/platform/arch/layering) holds the same table and reports the whole graph at once. Each block carries an `AGENTS.md` saying what it is and what it may reach for, and the substantial packages carry their own; the table at the end of [`AGENTS.md`](../AGENTS.md) indexes all of them. Two conventions are worth knowing before reading any of it: `internal/platform/db` is **generated** (edit `internal/platform/db/queries/*.sql` and run `make sqlc`), and handlers are thin — they parse, call a service, and render, so a rule added to a handler is a rule the assistant's tools never meet, since those tools call the same services directly.
...
Deeper: [`internal/ingest/sources`](../internal/ingest/sources/AGENTS.md) · [`internal/job/ghost`](../internal/job/ghost/AGENTS.md) · [`internal/application/userjob`](../internal/application/userjob/AGENTS.md) · [`internal/dict/skilltag`](../internal/dict/skilltag/AGENTS.md) · [`internal/dict/classify`](../internal/dict/classify/AGENTS.md)
...
Deeper: [`internal/search`](../internal/search/AGENTS.md) · [`internal/job/jobview`](../internal/job/jobview/AGENTS.md) · [`internal/job/ghost`](../internal/job/ghost/AGENTS.md)
...
Deeper: [`internal/candidate/experience`](../internal/candidate/experience/AGENTS.md) · [`internal/candidate/cvedit`](../internal/candidate/cvedit/AGENTS.md) · [`internal/candidate/cv`](../internal/candidate/cv/AGENTS.md) · [`internal/candidate/resume`](../internal/candidate/resume/AGENTS.md) · [`internal/candidate/pii`](../internal/candidate/pii/AGENTS.md)
...
Deeper: [`internal/ai/assistant`](../internal/ai/assistant/AGENTS.md) · [`internal/api/handler`](../internal/api/handler/AGENTS.md) · [`internal/platform/llm`](../internal/platform/llm/AGENTS.md) · [`internal/ai/llmkey`](../internal/ai/llmkey/AGENTS.md) · [`internal/ai/browsertools`](../internal/ai/browsertools/AGENTS.md)
...
Deeper: [`internal/identity/auth`](../internal/identity/auth/AGENTS.md) · [`internal/identity/auth/oauth`](../internal/identity/auth/oauth/AGENTS.md) · [`internal/identity/accounts`](../internal/identity/accounts/AGENTS.md)

## Notifications

Three independent delivery engines share one channel vocabulary. `internal/engage/notify` sends digests of new jobs matching a saved search; `internal/engage/reminder` nudges a saved job before it goes stale; `internal/engage/nudge` fires on lifecycle events — an application that has gone silent past its stage's threshold, or one that just moved into `interview`. Each has its own run-once cron worker (`cmd/notify`, `cmd/remind`, `cmd/nudge`).
...
Deeper: [`docs/agents/job-lifecycle.md`](agents/job-lifecycle.md) · [`internal/ingest/pipeline`](../internal/ingest/pipeline/AGENTS.md) · [`internal/platform/worker`](../internal/platform/worker/AGENTS.md)
...
|---|---|
| Adding a company or an ATS platform | [`internal/ingest/sources`](../internal/ingest/sources/AGENTS.md), [`README`](../README.md#adding-a-source) |
| The crawl, dedup, or board health | [`internal/ingest/pipeline`](../internal/ingest/pipeline/AGENTS.md) |
| Search, indexing, or a reindex incident | [`internal/search`](../internal/search/AGENTS.md), [`internal/search/searchdrain`](../internal/search/searchdrain/AGENTS.md) |
| HTTP routes, error rendering, middleware | [`internal/api/handler`](../internal/api/handler/AGENTS.md) |
| SQL, migrations, or sqlc | [`internal/platform/db`](../internal/platform/db/AGENTS.md) |
| Sessions, keys, OAuth | [`internal/identity/auth`](../internal/identity/auth/AGENTS.md), [`internal/identity/auth/oauth`](../internal/identity/auth/oauth/AGENTS.md) |
| The CV workspace | [`internal/candidate/cv`](../internal/candidate/cv/AGENTS.md), [`internal/candidate/cvedit`](../internal/candidate/cvedit/AGENTS.md), [`internal/candidate/experience`](../internal/candidate/experience/AGENTS.md) |
| The assistant, its tools or presets | [`internal/ai/assistant`](../internal/ai/assistant/AGENTS.md) |
